...
export const onRequest = async (context) => {
    ...
    if (url.pathname === '/api/config') { return next(); }  // /api/config 路由无需认证
    if (url.pathname === '/login') { return handleLogin(request, env); }  // 处理登录路由
    if (!config.password) { return next(); }  // 如果未设置密码，跳过认证
